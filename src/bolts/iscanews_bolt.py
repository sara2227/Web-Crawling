...
class WordCountBolt(Bolt):
    ...
    def get_news_content(self, url, headers={'User-Agent': 'Mozilla/5.0'}):
        ...
        if soup.find('div', {'class': 'item-body'}):
            if soup.find('div', {'class': 'item-body'}).find('div', {'class': 'item-text'}):
                body_images = soup.find('div', {'class': 'item-body'}).find('div', {'class': 'item-text'}).find_all(
                    'img')
                if len(body_images) != 0:
                    for image in body_images:
                        img_urls.append(image.get('src'))
            elif soup.find('div', {'class': 'item-body'}).find('img'):
                img_urls.append(soup.find('div', {'class': 'item-body'}).find('img').get('src'))
    ...
    def process(self, tup):
        ...
        if tup.values[0] != 'unknown_url':
            ...
            for item in titles:
                try:
                    time.sleep(1)
                    news_url = "http://www.iscanews.ir" + item.find('a').get('href')
