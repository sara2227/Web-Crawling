...
class WordCountBolt(Bolt):
    ...
    def get_news_content(self, url, headers={'User-Agent': 'Mozilla/5.0'}):
        ...
        if soup.find('ol', {'class': 'breadcrumb'}):
            services = soup.find('ol', {'class': 'breadcrumb'}).find('a').get_text()
            categories = soup.find('ol', {'class': 'breadcrumb'}).find('a').findNext('a').get_text()

        content = ''
        if soup.find('div', {'class': 'item-text'}):
            content = soup.find('div', {'class': 'item-text'}).get_text()
        elif soup.find('div', {'class': 'item-body'}):
            content = soup.find('div', {'class': 'item-body'}).get_text()

        img_urls = []
        if soup.find('div', {'class': "item-summary"}):
            if soup.find('div', {'class': "item-summary"}).find('img'):
                url = soup.find('div', {'class': "item-summary"}).find('img').get('src')
                img_urls.append(url)
        ...
        video_url = ""
        if soup.find('meta', {'property': 'video'}):
            video_url = soup.find('meta', {'property': 'video'}).get('content')
        elif soup.find('div', {'class': 'itemPlayer0'}):
            video_url = soup.find('div', {'class': 'itemPlayer0'}).find('source').get('src')

        return img_urls, summary.strip(), content.strip(), services.strip().replace(' ','‌'), categories.strip().replace(' ','‌'), video_url
    ...
    def process(self, tup):
        ...
        if tup.values[0] != 'unknown_url':
            ...
            archive = main_soup.find('section', {'id': "box517"})
            titles = []
            if archive:
            ...
            for item in titles:
                try:
                    time.sleep(1)
                    news_url = "https://www.mehrnews.com" + item.find('a').get('href')
                ...
                try:
                    ...
                    if news_url == "" or news_url == None or news_url is None:
                        news_url = "https://www.mehrnews.com"
